...
{#if showMenu}
    <div class='fixed top-0 left-0 w-screen h-screen bg-dark flex flex-col items-center justify-center gap-9 z-30 text-white' transition:fade>
...
<nav class=' absolute z-10 top-0 left-0 w-screen'>
    <ContentWidth class='flex flex-row justify-between items-center border-b-1 py-5 border-white relative'>
        ...
        <button onclick={()=>showMenu=!showMenu} aria-label='toggle menu' class='z-40 pr-2 block md:hidden'><i class='fa-solid fa-sharp fa-bars fa-2xl text-white hover:text-light active:transition-none active:text-dark transition-colors duration-500'></i></button>
